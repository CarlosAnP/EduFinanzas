...
from education.models import Challenge, Guide, Quiz, Question, Choice, UserChallenge
...
def populate():
    ...
    for m in missions:
        ...
        print(f"   [Trigger: {m['trigger_type']}] {m['title']} (+{m['reward_points']} pts)")
    ...
    print(f"   Quiz: Regla 50/30/20 creado")
    ...
    print(f"   Quiz: Fondo de Emergencia creado")

    print("\n--- Modulo educativo poblado con exito ---")
    print(f"   - {Challenge.objects.count()} misiones creadas")
    print(f"   - {Guide.objects.count()} guias creadas")
    print(f"   - {Quiz.objects.count()} quizzes creados")
